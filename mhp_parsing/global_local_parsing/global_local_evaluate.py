...
def get_arguments():
    ...
    parser.add_argument("--save-results", default=True, action="store_true", help="whether to save the results.")
...
def glparsing(data_dir,split_name,schp_ckpt,log_dir):
    ...
    # Data loader
    local_test_dataset = CropDataValSet(data_dir, split_name[0], crop_size=input_size, transform=transform,
                                      flip=args.flip)
    local_num_samples = len(local_test_dataset)
    print('local testing sample numbers: {}'.format(local_num_samples))
    local_testloader = data.DataLoader(local_test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)

    global_test_dataset = CropDataValSet(data_dir, split_name[1], crop_size=input_size, transform=transform,
                                      flip=args.flip)
    global_num_samples = len(global_test_dataset)
    print('global testing sample numbers: {}'.format(global_num_samples))
    global_testloader = data.DataLoader(global_test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(local_testloader)):
            ...
            if args.save_results:
                ...
                logits_result_path = os.path.join(local_sp_results_dir, im_name + '.npy')
        ...
        for idx, batch in enumerate(tqdm(global_testloader)):
            ...
            if args.save_results:
                ...
                # save logits
                logits_result = transform_logits(logits, c, s, w, h, input_size)
                logits_result_path = os.path.join(global_sp_results_dir, im_name + '.npy')
                np.save(logits_result_path, logits_result)
    return
