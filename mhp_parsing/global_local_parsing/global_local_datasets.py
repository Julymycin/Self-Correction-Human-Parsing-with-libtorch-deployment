...
class CropDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_path = os.path.join(self.root, self.split_name + '_images',
                               train_item + '.jpg')
        parsing_anno_path = os.path.join(self.root,
                                         self.split_name + '_segmentations',
                                         train_item + '.png')
...
class CropDataValSet(data.Dataset):
    def __init__(self,
        ...
        list_path = os.path.join(self.root, self.split_name)
        val_list = [i_id for i_id in sorted(os.listdir(list_path))]
    ...
    def __getitem__(self, index):
        val_item = self.val_list[index][:-4]
        # Load training image
        im_path = os.path.join(self.root, self.split_name,
                               val_item + self.val_list[index][-4:])
        # im_path = os.path.join(self.root, self.split_name, val_item)
        im = cv2.imread(im_path, cv2.IMREAD_COLOR)
        h, w, _ = im.shape
        # Get person center and scale
        person_center, s = self._box2cs([0, 0, w - 1, h - 1])
...
class CropDataSetfromArray(data.Dataset):
    def __init__(self,
                 root,
                 split_name='crop_pic',
                 crop_size=[473, 473],
