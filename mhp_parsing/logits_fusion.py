...
def multi_process(a,gp_dir,lp_dir,mask_dir,save_dir):
    ...
    gt_output = patch2img_output(lp_dir, img_name, img_height, img_width, msrcnn_bbox, bbox_type='msrcnn',
                                 num_class=7)
...
#     fused_output = global_output + msrcnn_output + gt_output
    ...
    mask_output_path = os.path.join(mask_dir, os.path.splitext(img_name)[0] + '_mask.npy')
    result_saving(fused_output, img_name, img_height, img_width, save_dir, mask_output_path, bbox_score, msrcnn_bbox)
    return


def gl_fuse(crop_json,gp_dir,lp_dir,mask_dir,save_dir,):
    json_file = open(crop_json)
    anno = json.load(json_file)['root']

    results = joblib.Parallel(n_jobs=24, verbose=10, pre_dispatch="all")(
        [joblib.delayed(multi_process)(a,gp_dir,lp_dir,mask_dir,save_dir) for i, a in enumerate(anno)]
    )
