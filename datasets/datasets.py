...
class PascalDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_path = os.path.join(self.root, 'JPEGImages', train_item + '.jpg')
        parsing_anno_path = os.path.join(self.root, 'pascal_person_part_gt', train_item + '.png')
        ...
        if self.dataset != 'test':
            # Get pose annotation
            parsing_anno = cv2.imread(parsing_anno_path, cv2.IMREAD_GRAYSCALE)
            parsing_anno = np.where(parsing_anno == 75, 1, 0)  # for torso
            if self.dataset == 'train' or self.dataset == 'trainval':
                sf = self.scale_factor
                rf = self.rotation_factor
                s = s * np.clip(np.random.randn() * sf + 1, 1 - sf, 1 + sf)
